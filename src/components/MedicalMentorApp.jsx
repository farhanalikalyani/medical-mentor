  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard label="Overall Accuracy" value={`${accuracy}%`} sub={`${correct}/${total} correct`} icon={Target} color="bg-cyan-600" />
        <StatCard label="Cards Due Today" value={dueCards} sub="spaced repetition" icon={CreditCard} color="bg-purple-600" />
...
export default function MedicalMentorApp({ studentName = "there", onAdmin }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white lg:flex">
      {/* Desktop sidebar — replaces the bottom nav on wide screens */}
      <aside className="hidden lg:flex lg:flex-col lg:w-60 lg:flex-shrink-0 lg:h-screen lg:sticky lg:top-0 border-r border-slate-800/60 px-4 py-6">
        <div className="flex items-center gap-2 mb-8 px-1">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-500 to-teal-600 flex items-center justify-center">
            <Stethoscope size={16} className="text-white" />
          </div>
          <span className="text-base font-bold text-white">Medical Mentor</span>
        </div>

        <div className="flex items-center gap-1.5 bg-slate-800/60 rounded-full px-3 py-1.5 border border-slate-700/40 mb-6 w-fit">
        ...
        <nav className="flex flex-col gap-1 flex-1">
          {NAV_ITEMS.map(({ id, label, icon: Icon }) => (
            ...
            >
              <Icon size={17} strokeWidth={screen === id ? 2.4 : 1.8} />
              {label}
            </button>
          ))}
        </nav>
        ...
        {onAdmin && (
          <button
            onClick={onAdmin}
            className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs font-medium text-slate-600 hover:text-slate-300 hover:bg-slate-900/60 transition-colors"
          >
      ...
      <div className="flex-1 flex flex-col lg:min-h-screen">
        {/* Mobile-only top bar */}
        <header className="flex lg:hidden items-center justify-between px-4 pt-4 pb-2 flex-shrink-0">
          <div className="flex items-center gap-2">
        ...
        {/* Main content — genuinely wide on desktop, not boxed */}
        <main className="flex-1 overflow-y-auto px-4 pt-2 pb-20 lg:px-10 lg:py-8 lg:pb-8">
        ...
        <nav className="lg:hidden fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-slate-900/95 backdrop-blur border-t border-slate-800/80 flex z-50">
          {NAV_ITEMS.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              onClick={() => setScreen(id)}
